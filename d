...
local function mouseclickmiddle()
    local delay = 0.05
    ...
    VirtualInputManager:SendMouseButtonEvent(centerX, centerY, 0, true, game, false)
    task.wait(delay)
    VirtualInputManager:SendMouseButtonEvent(centerX, centerY, 0, false, game, false)
end
                    ...
                    if predictedPosition and isWithinBoxFOV(predictedPosition) and os.clock() - lastClickTime >= getgenv().Forbidden.TriggerBot.Settings.Cooldown then
                        ...
                        if tool and tool:IsA("Tool") then
                            ...
                            if shootFunction and shootFunction:IsA("RemoteEvent") then
                                ...
                                Forlorn.mouse1press(screenPos.X, screenPos.Y)
                                Forlorn.mouse1release(screenPos.X, screenPos.Y)
                            end
                            return
                        end
                    end
